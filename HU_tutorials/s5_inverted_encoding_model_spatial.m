...
% this is broken up into several subtasks - the first of which is generating 
% a stimulus mask. Because the stimulus is a circle w/ a known, jittered location 
% on each trial, we can just generate a binary image of 0's and 1's, with 1's 
% at the positions subtended by the stimulus and 0's elsewhere. A similar technique 
% is used by those mapping population receptive fields (pRFs).(Dumoulin & Wandell, 
% 2008; Kay et al, 2013)
% 
% This trial-by-trial stimulus mask will then be projected onto each channel's 
% filter (a 2-d Gaussian-like function, usually). This amounts to a reduction 
% in dimensionality from n_pixels to n_channels, though the transformation is 
% necessarily lossy in this framework
% 
% Use the trn_conds coordinates (first column is x, second column is y), 
% and the stimulus size of 1.449 dva to generate a stimulus mask of 1's and 0's. 
% The visual field area subtended by the display was 16.128 dva horizontally and 
% 9.072 dva vertically (full width/height edge-to-edge).
% 
% At this stage, it doesn't matter if we overestimate the size of the screen, 
% so for simplicity let's say the screen is 10 dva tall and 17 dva wide.
%%
...
% an encoding model describes how a neural unit responds to an arbitrary set 
% of stimluli. A common encoding model for visual stimuli measured for decades 
% in the spatial receptive field (RF). Rather than directly estimate the exact 
% receptive field of each voxel, we'll instead suppose that there are a discrete 
% number of possible RF positions (that is, a discrete number of spatial filters) 
% which describe discrete neural subpopulations. Each voxel then can be modeled 
% as a weighted sum of these discrete subpopulations. While this is of course 
% an approximation of the actual nature of RF properties in the visual system, 
% by supposing a small number of possible RF positions/subpopulations we can *invert* 
% the estimated encoding model (which is a set of weights on each supopulation 
% for each voxel) in order to reconstruct the activation of each population given 
% a novel voxel activation pattern observed (below). 
% 
% First, let's generate the filters corresponding to our hypothetical neural 
% populations. We'll assume, for simplicity, that they are a square grid and are 
% all the same "size" (FWHM). If interested, try varying these parameters. 
%%
...
% channels, we want to determine the contribution of each information channel 
% (hypothetical neural population) to each voxel. We set up the problem like a 
% standard general linear model often used for fMRI activation map-based analyses: 
% y = beta*X, where: - y is the measured signal in a voxel across all trials (1 
% x n_trials) - beta is the weights corresponding to how much each channel's response 
% contributes to each voxel (stable across trials, 1 x n_channels) - X is a design 
% matrix, which is how much each hypothetical population, with its defined RF, 
% should respond to the stimulus (which we know) (n_trials x n_channels)
% 
% note that X is the same design matrix used for any standard GLM-based analysis 
% in fMRI - we're just defining our "predictors" a bit differently. Here, predictors 
% are predicted channel responses to known stimuli on each trial (the stimulus 
% mask above). 
% 
...
% Now that we have our design matrix and our measured signal, we need to solve 
% for the weights ("betas" in above equation). Because we have more trials than 
% channels, and because our stimulus positions are unique on each trial, we should 
% be able to estimate a unique solution for the channel weights given our observed 
% activation pattern using ordinary least squares regression. 
%%
...
% Our estimated "w" above reflects an ENCODING MODEL for each voxel. For a given 
% region (here, V1), we have an encoding model estimated for each voxel. Given 
% our linear framework described above, we can INVERT the encoding model in order 
% to estimate the channel activation pattern (like our trnX) which is associated 
% with any novel activation pattern that is observed. The associated mapping from 
% VOXEL SPACE into CHANNEL SPACE is computed from the pseudoinverse of the estimated 
% weight matrix.
% 
% This inversion step is where we get "inverted" encoding model from - we're 
% not just estimating encoding models for single voxels and comparing them; we're 
% instead estimating all encoding models, and using the joint set of encoding 
% models to constrain a mapping from voxel space into channel space. 
% 
% Once we have the inverted mapping (the pseudoinverse), we can map the novel 
% activation pattern ("tst") back into channel space.
%%
...
% it is useful to compute a stimulus reconstruction that can be more easily quanitfied 
% by, e.g., curvefitting methods. Furthermore, the discrete values above are challenging 
% to "coregister" across different stimulus position conditions (at least when 
% stimuli are not simply at the same position on different sides of the screen). 
% Take for example a scenario like Sprague, Ester & Serences, 2014 - during a 
% spatial WM task, a remembered stimulus appeared at a random position along a 
% ring of a given eccentricity. Because we were not interested in whether or not 
% representations for stimuli at different positions were different from one another, 
% we sought to average representations for stimuli at different positions together. 
% We did this by reconstructing an "image" of the contents of WM via a weighted 
% sum of the spatial filters for each channel. Let's try that here.
% 
% We have our basis_set, used to compute predicted channel responses, and 
% we have our estimated channel responses (chan_resp) - to compute the weighted 
% sum as an image, we just need to multiply the two.
%%
...
% One of the biggest advantages of the IEM technique is that by transforming 
% activation patterns into a "feature space" (here, spatial information channels), 
% you have full knowledge of how your activation is related to feature values. 
% Put another way, you know how to manipulate your feature space in order to coregister 
% non-like trials. For example, here, we don't necessarily care too much about 
% what may or may not be different in stimulus reconstructions computed from trials 
% in which stimuli were on the left compared with trials when stimuli were on 
% the right. Accordingly, we can average these trials after "coregistering" the 
% reconstructions. Fortunately, this is an easy dataset to coregister. We just 
% need to flip the reconstructions across the vertical meridan for one half of 
% trials (sorted by trn_conds(:,1), which is the stimulus side). It doesn't matter 
% whether we flip left trials onto the right side of the screen or vice versa. 
% Try it below!
%%
